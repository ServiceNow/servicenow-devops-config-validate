...
const main = async() => {
    try {
        ...
        if ("true" !== String(autoCommit).toLowerCase()) {
            info(`The auto-commit flag is set to ${String(autoCommit)}. Further evaluation of the action is stopped.`)
            return;
        }
        // Stop further evaluation of the action if autoValidate is not true
        else if ("true" !== String(autoValidate).toLowerCase()) {
            info(`The auto-validate flag is set to ${String(autoValidate)}. Further evaluation of the action is stopped.`)
            return;
        }
        ...
        {
            info(`The auto-publish flag is set to ${String(autoPublish)}. No snapshot will be published in this action.`)
        }
        ...
        if("true" === String(terminateOnPolicyValidationFailures).toLowerCase() && !constants.SNAPSHOT.VALIDATION_PASS_STATES.includes(validationStatus))
            throw new Error(`Validation for the snapshot '${snapshotRec.name}' did not succeed. The validation status is ${validationStatus}.`)
