...
async function isDeployableImpacted(snInstanceURL, changesetNumber, deployableName) {
    ...
    if (!isArrayEmpty(otherImpactedDeployables)) {
        ...
        {
            warning(`The config data is uploaded and will be validating against the deployable '${deployableName}'. However the impacted deployables are '${deployableName}, ${otherImpactedDeployables.map(dep => dep.name).join(', ')}'.`);
        }
        else
        {
            warning(`Deployable '${deployableName}' is not impacted with ${changesetNumber}. However the impacted deployables are '${otherImpactedDeployables.map(dep => dep.name).join(', ')}' against which no validation will be performed.`);
        } 
    }
